...
class wrapNodes extends Plugin {
    constructor() {
         ...
         */
        this.isSuitableStart = (n) => (Dom.isText(n) && isString(n.nodeValue) && /[^\s]/.test(n.nodeValue)) ||
            (this.isNotClosed(n) && !Dom.isTemporary(n));
        /**
         * Node should add in block element
         */
        this.isSuitable = (n) => Dom.isText(n) || this.isNotClosed(n);
        /**
         * Some element which need append in block
         */
        this.isNotClosed = (n) => Dom.isElement(n) &&
